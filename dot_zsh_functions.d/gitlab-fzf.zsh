...
gitlab_fzf() {
  local selected pipeline_id pipeline_status ref action answer
  ...
  IFS='|' read -r pipeline_id pipeline_status ref _ <<<"$selected"
  ...
  case "$action" in
    ...
    cancel)
      read -r "answer?Annuler le pipeline ${pipeline_id} ? [y/N] "
