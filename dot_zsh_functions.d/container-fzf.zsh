...
_container_fzf_containers() {
  local selected id name container_status image action answer
  ...
  [[ -n "$selected" ]] || return 1
  IFS='|' read -r id name container_status image <<<"$selected"
  ...
  case "$action" in
    ...
    remove)
      read -r "answer?Supprimer ${name} ? [y/N] "
...
_container_fzf_images() {
  ...
  case "$action" in
    ...
    verify)
      ...
      ;;
    remove)
      read -r "answer?Supprimer l’image ${reference} ? [y/N] "
      [[ "$answer" =~ ^[YyOo]$ ]] || return 1
