...
_aws_fzf_continue_command() {
  ...
  words=("${(@f)$(_aws_fzf_command_words "$line")}")
  service="${words[2]:-}"
  operation="${words[3]:-}"
...
_aws_fzf_widget() {
  local left command completion mode new_left trimmed_left
    ...
    if _aws_fzf_option_needs_value "$service" "$operation" "$option"; then
      case "$option" in
        --output|--color|--cli-binary-format|--profile)
          ...
          ;;
        *)
        if ! read -r "value?Valeur pour ${option} : "; then
          printf '\n' >&2
          return 1
        fi
          ;;
...
_aws_fzf_confirm_sensitive_operation() {
  ...
  case "$operation" in
    delete-*|deregister-*|disable-*|remove-*|revoke-*|stop-*|terminate-*)
      read -r "answer?Opération sensible (${operation}). Continuer ? [y/N] "
      [[ "$answer" =~ ^[YyOo]$ ]]
      ;;
    *)
      return 0
...
if [[ -o interactive ]]; then
  zle -N _aws_fzf_widget
  bindkey '^G' _aws_fzf_widget
fi
