...
_argocd_choose_command() {
  ...
  local -a command_path=() fzf_options

  fzf_options=("${(@f)$(_argocd_fzf_base_options)}")

  while :; do
    rows="$(_argocd_subcommands "${command_path[@]}")" || return
    [[ -n "$rows" ]] || break

    prefix="$(_argocd_join_command "${command_path[@]}")"
    if (( ${#command_path[@]} == 0 )); then
      prompt='Commande Argo CD > '
    else
      prompt="Action pour ${command_path[*]} > "
    fi
    ...
    command_path+=("${selection%%|*}")
  done

  _ARGOCD_FZF_COMMAND=("${command_path[@]}")
}
...
_argocd_collect_arguments() {
  ...
  group="${_ARGOCD_FZF_COMMAND[1]:-}"
  action="${_ARGOCD_FZF_COMMAND[2]:-${_ARGOCD_FZF_COMMAND[1]:-}}"
  arguments=("${(@f)$(
  ...
  for raw_argument in "${arguments[@]}"; do
    ...
    if (( first_argument )) &&
      ...
      selected_resources=("${(@f)selected}")
      _ARGOCD_FZF_ARGUMENTS+=("${selected_resources[@]}")
    else
      if ! read -r "value?Valeur pour ${label%...} : "; then
        printf '\n' >&2
        return 1
      fi
      [[ -n "$value" ]] || {
...
_argocd_build_flags() {
  ...
  for row in "${rows[@]}"; do
    ...
    if [[ -n "$value_type" ]]; then
      if ! read -r "value?Valeur pour ${flag} (${value_type}) : "; then
        printf '\n' >&2
        return 1
      fi
      [[ -n "$value" ]] || {
